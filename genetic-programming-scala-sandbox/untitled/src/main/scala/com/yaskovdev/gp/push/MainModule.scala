...
object MainModule {
  ...
  case object AUXILIARY extends PushType[Program] {
    val name = Symbol("AUXILIARY")

    def getStack(ps: ProgramState): Stack[Program] = ps.AUXILIARY

    def modifyStack(ps: ProgramState, newStack: Stack[Program]): ProgramState = ps.copy(AUXILIARY = newStack)
  }

  case class Instruction(name: Symbol) {
    implicit def convert(s: Symbol): Instruction = Instruction(s)
  }
  ...
  sealed abstract class Program {
    override def toString: String = programToString(this)
  }
  ...
  sealed case class ProgramState(
    ...
    def pop[T](PUSHTYPE: PushType[T]): ProgramState =
      this match {
        case ps@PUSHTYPE(_ :: tl) => ps set(PUSHTYPE, tl)
        case _ => this
      }
  }
  ...
  (add instruction FLOAT) ('FROMINTEGER)(conversion(INTEGER)(FLOAT)(a => a.toFloat))
  ...
  def runRush(code: Program, state: ProgramState, shouldPrint: Boolean): ProgramState = {
    val stateWithPushed =
      if (TOP_LEVEL_PUSH_CODE) {
        state push(CODE, code)
      } else {
        state
      }
    ...
    val stateWithPopped =
      if (TOP_LEVEL_POP_CODE) {
        stateEvaluated pop CODE
      } else {
        stateEvaluated
      }
    stateWithPopped
  ...
  def autoSimplify(toSimplify: Program, errorFunction: Program => List[Float], steps: Int, shouldPrint: Boolean, progressInterval: Int) = {
    if (shouldPrint) {
      println("Auto-simplifying with starting size: " + countPoints(toSimplify).toString)
    }
  ...
  def pushgp(cfg: GpConfig): Option[Individual] = {
    ...
    def mainLoop(generation: Int,
      ...
      if (generation <= MAX_GENERATIONS) {
        ...
        if (best.totalError.isDefined && best.totalError.get <= cfg.errorThreshold) {
          ...
          val simplified = autoSimplify(cfg)(best.program, shouldPrint = false, 1000)
  ...
  def getPopulationWithScaledErrors(cfg: GpConfig)(basePop: List[Individual]): List[Individual] = {
    ...
    val totalSuccessesPerErrorCase = ((0 to numErrorsPerIndividual).par map { errorIndex =>
      basePop count (_.errors(errorIndex) <= perCaseThreshold)
    }).toList
    ...
    (0 to cfg.populationSize).par map {
      individualIdx =>
        ...
        if (n < probs(1))
          ...
          autoSimplify(cfg)(newIndividual.program, shouldPrint = false, 1000)
        else
          newIndividual
    } toList
  }
    ...
    pushgp(GpConfig(
      ...
      atomGenerators = ((add.registeredForType(INTEGER) :::
        ...
        List(
          Generator(() => randInt(100): Program),
          Generator(() => randFloat(): Program),
          Generator(() => Symbol("AUXILIARY_IN"): Program)
        ): List[AtomGenerator]),
      maxPoints = 100,
      scaleErrors = true
    ))
